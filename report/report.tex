...
We trained our net for 50 epochs across the 5 folds. The validation step and the metrics (train and validation accuracy, loss and f1) are computed after each epoch. After each fold the model is saved. At the end of the training step we save the best model and all the train metrics in a dataframe. We also calculate the mean and the standard deviation of the accuracy across all the folds.
    ...
    \begin{table}[h]
        ...
        \hline
        \multicolumn{1}{|c|}{Fold}& \multicolumn{1}{c|}{F1 (Val)} & \multicolumn{1}{c|}{F1 (Train)} & \multicolumn{1}{c|}{Accuracy (Val)} & \multicolumn{1}{c|}{Accuracy (Train)} & \multicolumn{1}{c|}{Loss (Val)} \\ \hline
        fold 1  & 0.92625     & 0.932328                   & 0.909188          & 0.915203  & 0.387399                \\ \hline
        fold 2  & 0.938097    & 0.930022                   & 0.921449          & 0.912906  & 0.392215                \\ \hline
        fold 3  & 0.921491    & 0.92541                    & 0.901728            & 0.907666  & 0.404772                \\ \hline
        fold 4  & 0.910947    & 0.921576                   & 0.892681          & 0.903138  & 0.411598                \\ \hline
        fold 5  & 0.934723    & 0.937001                   & 0.918551          & 0.924725  & 0.37091               \\ \hline
        \end{tabular}
        \caption{CNN mean metrics (F1 score, accuracy and Loss) for each fold in train and test}
    \end{table}
...
\begin{table}[h]
    ...
    fold 5  & 0.934723    & 0.937001                   & 0.918551          & 0.924725  & 0.37091               \\ \hline
    \end{tabular}
    \caption{ Scatnet mean metrics (F1 score, accuracy and Loss) for each fold in train e test}
\end{table}
...
As we already said, in this case the filters are known and not learned during the training phase. We
can visualize them using the Fourier transform:
\begin{figure}[H]
    \centering
